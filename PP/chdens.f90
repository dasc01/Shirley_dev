...
!---------------------------------!
  ...
  NAMELIST /plot/  &
       ...
       radius, output_format, fileout, plot_center_atom
...
!!$  !DASb
!!$  !---das tests : potential from charge density difference------
!!$  !
!!$  gamma_only=.false.
!!$  !
!!$  CALL allocate_fft()
...
!!$  ALLOCATE  (rhos(nr1x*nr2x*nr3x))
!!$  rhos(:)=0.0d0
!!$!  CALL vh_psolver(rhor, ehart, charge, rhos)
!!$  CALL v_h( rhog, ehart, charge, rhos )
!!$  rhor(:)=rhos(:)
!!$  DEALLOCATE (rhos)  
!!$  DEALLOCATE (rhog)  
!!$  WRITE(6,*) "ehart, charge=",ehart, charge
!!$  !DASe
!!$  !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!       nproc_pool=1
  ...
  ELSEIF (iflag == 3) THEN
     ...
     IF (output_format == 5.and.ionode) THEN
        ...
        if( plot_center_atom < 1 .or. plot_center_atom > nat) then
          write(stdout,*) ' centering grid at center of mass'
          call all_center( tauc, shift )
        else
          write(stdout,*) ' centering grid at atom ', plot_center_atom
        ...
        ! XCRYSDEN FORMAT
        !
        CALL xsf_struct (alat, at, nat, tauc, atm, ityp, ounit)
     ...
     ELSEIF (output_format == 6.and.ionode ) THEN
        ...
        !
        CALL write_cubefile (alat, at, bg, nat, tauc, atm, ityp, rhor, &
  ...
  allocate( grid(nr1x, nr2x, nr3x) )
  grid = reshape( rhor, (/ nr1x, nr2x, nr3x /) )
  rhor = 0.d0
  do i3=1,nr3
