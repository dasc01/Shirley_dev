...
200  CALL errore ('shirley_ham', 'reading input namelist', ABS (ios) ) 
  ...
  CALL mp_bcast( ksplord,  ionode_id )
  CALL mp_bcast( ncpp, ionode_id )
  CALL mp_bcast( updatepp, ionode_id )
  CALL mp_bcast( pwmtxel_method, ionode_id )
  CALL mp_bcast( pwmtxel_cutoff, ionode_id )
  CALL mp_bcast( debug, ionode_id )
  CALL mp_bcast( reduced_io, ionode_id )
  CALL mp_bcast( band_subset, ionode_id )
  CALL mp_bcast( triples_subset, ionode_id )
  ! 
  CALL mp_bcast( nplot, ionode_id )
  CALL mp_bcast( plot_prefix, ionode_id )
  CALL mp_bcast( plot_style, ionode_id )
  CALL mp_bcast( plot_center_atom, ionode_id )
  ! 
  CALL mp_bcast( nkgrid,   ionode_id )
  CALL mp_bcast( ikgrid,   ionode_id )
  !
  CALL mp_bcast( nspin_ham, ionode_id )
  !
  if( updatepp ) then
    CALL mp_bcast( pseudo_dir, ionode_id )
    CALL mp_bcast( nspecies, ionode_id )
    if( .not. ionode ) allocate( local_channel(nspecies), pseudo_file(nspecies) )
    CALL mp_bcast( local_channel, ionode_id )
    CALL mp_bcast( pseudo_file, ionode_id )
  endif

  ! elphmtxel
  call mp_bcast( ibuf5, ionode_id )
  call mp_bcast( dvfile, ionode_id )
  call mp_bcast( elphfile, ionode_id )
  call mp_bcast( dynfile, ionode_id )
  nrdv1   = ibuf5(1)
  nrdv2   = ibuf5(2)
  nrdv3   = ibuf5(3)
  nspindv = ibuf5(4)
