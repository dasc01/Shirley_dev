  ...
  USE gvect, ONLY : ngm, ngm_g, gcutm, g, ig_l2g, nl
  ...
  USE cell_base,            ONLY : at, bg, ibrav, celldm, alat, tpiba, tpiba2, &
         omega, symm_type
  USE wvfct,         ONLY : ecutwfc, nbnd, npwx, npw, igk, et, wg, nbndx
  USE wavefunctions_module, ONLY : evc
  use noncollin_module, only : npol
  USE io_files,      ONLY : iunwfc, nwordwfc, iunigk, prefix, tmp_dir, diropn
  use input_parameters, only : pseudo_dir
  USE mp, ONLY : mp_max, mp_min, mp_barrier, mp_sum, mp_bcast
  use mp_global, only : kunit, mpime
  ...
  USE wfc_shirley, only : expand_Gspace, &
                          init_shirley_wfc_1, init_shirley_wfc_2, &
                          reorder_shirley_wfc, write_shirley_wfc, &
                          read_shirley_wfc_1, read_shirley_wfc_2, &
                          dotprod_shirley_wfc, npw_gamma, ecutwfc_gamma, &
                          evc_1, evc_2, map_shirley_bz, &
  ...
  ! store the number of k-points as it will be redefined at 1
  nkstot_in=nkstot
  call expand_Gspace( ecut_in )
  ! from now on nkstot=1

  call init_shirley_wfc_1

!  allocate( phase(nbnd), vc(npwx) )

  ! reorder wave function coefficients using ordering at Gamma-point
  do ik=1,nkstot_in

    write(stdout,*) ' reordering k-point ', ik
    call flush_unit(stdout)

    ! load wave functions for ik
    ! note that we require 2*nwordwfc in v4.3
    write(stdout,*) ' size evc = ', size(evc), nwordwfc
    call davcio( evc, 2*nwordwfc, iunwfc, ik, -1 )

!    ! new fixing of phase
!    if( ionode ) then
!      write(stdout,*) ' fixing phases'
!      do ibnd=1,nbnd
!        vc = abs(evc(:,ibnd))
!        imax=maxloc(vc)
!        phase(ibnd)=evc(imax(1),ibnd)
!        phase(ibnd)=conjg(phase(ibnd))/abs(phase(ibnd))
!      enddo
!    endif
!    call mp_bcast( phase, ionode_id )
!    do ibnd=1,nbnd
!      evc(:,ibnd)=evc(:,ibnd)*phase(ibnd)
!    enddo
      
    ! reorder coefficients
    call reorder_shirley_wfc( ik )

    ! write reordered wave functions
    call write_shirley_wfc( ik )

  enddo

!  deallocate( phase, vc )

  ! don't need evc anymore so deallocate it
  if( allocated(evc) ) deallocate( evc )
  ! and allocate another shirley wavefunction array
  call init_shirley_wfc_2

  ! initialize matrices for Shirley basis
  call basis_init( nkstot_in, nbnd )

  ! construct overlap
  call construct_overlap( nkstot_in, nbnd )
  ...
  ! reallocate evc
  allocate( evc(npw_gamma,nbasis_trunc), stat=ierr )
  if( ierr/=0 ) call errore('build_optimal_basis','unable to allocate space for final optimal basis set',1)
