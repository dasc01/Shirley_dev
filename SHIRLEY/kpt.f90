...
! ----------------------------------------------------------------------
  subroutine kpt_scatter( kptlist, kptparam, root )
! ----------------------------------------------------------------------
  ...
  use mp, only : mp_bcast, mp_barrier, mp_scatter_size, mp_scatter
  ...
  call mp_bcast( kptparam%cartesian, root )
  call mp_bcast( kptparam%grid_type, root )
  call mp_bcast( kptparam%nktot,     root )
  call mp_bcast( kptparam%nkgrid,    root )
  call mp_bcast( kptparam%ikgrid,    root )
  call mp_bcast( kptparam%shift_type,root )
  call mp_bcast( kptparam%shift,     root )
...
! ----------------------------------------------------------------------
  subroutine kpt_bcast( kpt, root )
! ----------------------------------------------------------------------

  type(kpt_type) :: kpt
  integer,intent(in) :: root

  call kptparam_bcast( kpt%param, root )
  if( trim(kpt%param%grid_type) == 'bandstructure' ) then
    call kptbandstructure_bcast( kpt%bandstructure, root )
  endif
  call kptlist_bcast( kpt%list, root )
  if( trim(kpt%param%grid_type) == 'tetrahedra' ) then
    call kpttetra_bcast( kpt%tetra, root )
  endif
...
! ----------------------------------------------------------------------
  subroutine kptparam_bcast( kptparam, root ) 
! ----------------------------------------------------------------------
...
! ----------------------------------------------------------------------
  subroutine kptbandstructure_bcast( kptbs, root ) 
! ----------------------------------------------------------------------
  ...
  call mp_bcast( kptbs%nksp, root )
  call mp_bcast( kptbs%kres, root )
  call mp_bcast( kptbs%nk, root )
  isassoc = .false.
  if( mpime == root ) then
  ...
  call mp_bcast( kptbs%xksp, root )
  call mp_bcast( kptbs%labelsp, root )
  call mp_bcast( kptbs%kpathlensp, root )
  call mp_bcast( kptbs%kpathlen, root )

  end subroutine kptbandstructure_bcast

 
! ----------------------------------------------------------------------
  subroutine kptlist_bcast( kptlist, root ) 
! ----------------------------------------------------------------------
  ...
  call mp_bcast( kptlist%nk,    root )
  call mp_bcast( kptlist%nspin, root )
  isassoc = .false.
  if( mpime == root ) then
  ...
  if( isassoc(1) ) call mp_bcast( kptlist%kvec, root )
  if( isassoc(2) ) call mp_bcast( kptlist%wk, root )

  end subroutine kptlist_bcast


! ----------------------------------------------------------------------
  subroutine kpttetra_bcast( kpttetra, root ) 
! ----------------------------------------------------------------------
  ...
  call mp_bcast( kpttetra%ntetra, root )
  isassoc = .false.
  if( mpime == root ) then
  ...
  call mp_bcast( kpttetra%tetra, root )
