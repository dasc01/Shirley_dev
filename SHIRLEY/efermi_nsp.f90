...
! debug
  ...
  allocate( eigval(nbnd,nk_loc*nspin), tmpeig(nbnd), &
            wg(nbnd,nk_loc*nspin), wk_loc(nk_loc*nspin), stat=ierr )
...
!     write(stdout,*)  ' nele_sp=', nele_sp
  nele_sp=nelec
     ...
     do ik=1,nk
        ...
        write(stdout,*) ' reading ik= ', ik, ' of ', nk, ' wt= ', wk_loc(nk_loc), ' ispin=', ispin
  ...
  ! close - otherwise I don't know if I have the data or not
     ...
     call fermi_energy_sub( nele_sp, nk_loc, nbnd, wk_loc, eigval, kT, smearing, ef , 1)
