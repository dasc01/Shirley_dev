...
!DASb
    gwrun=.false.
    qpwrite=3  !1=pbe, 2=gw-pert, 3-gw, 4=hf
!DASe
...
!DASb
  call mp_bcast( gwrun, ionode_id )
  call mp_bcast( qpwrite, ionode_id )
!DASe

  call mp_bcast( prefix, ionode_id )
  call mp_bcast( outdir, ionode_id )

  call mp_bcast( pltfile, ionode_id )
  ...
  call mp_bcast( readqpts, ionode_id )
  call mp_bcast( readcorerep, ionode_id )
  call mp_bcast( readplotspec, ionode_id )
  call mp_bcast( pseudo_dir, ionode_id )
  call mp_bcast( trifile, ionode_id )
  call mp_bcast( int0file, ionode_id )
  call mp_bcast( vxcfile, ionode_id )

  call mp_bcast( epsfile, ionode_id )
  call mp_bcast( eps00file, ionode_id )
  call mp_bcast( epsijfile, ionode_id )
  call mp_bcast( eps0jfile, ionode_id )
  call mp_bcast( epsi0file, ionode_id )
  call mp_bcast( epsqfile, ionode_id )

  call mp_bcast( deltaq, ionode_id )
  call mp_bcast( epstol, ionode_id )
  call mp_bcast( band_subset, ionode_id )
  call mp_bcast( nener_sigma, ionode_id )
  call mp_bcast( dener_sigma, ionode_id )

  call mp_bcast( niter_equil_mc, ionode_id )
  call mp_bcast( niter_samp_mc, ionode_id )
  call mp_bcast( mc_step_radius, ionode_id )

  call mp_bcast( spec_min, ionode_id )
  call mp_bcast( spec_max, ionode_id )
  call mp_bcast( spec_sigma, ionode_id )
  call mp_bcast( spec_smearing, ionode_id )

  call mp_bcast( omega_resp, ionode_id )
  call mp_bcast( eta_resp, ionode_id )
  call mp_bcast( plot_style, ionode_id )

  call mp_bcast( nspline_fit, ionode_id )
  call mp_bcast( spline_fit, ionode_id )

  call mp_bcast( niter_stieltjes, ionode_id )
  call mp_bcast( nener_stieltjes, ionode_id )
  call mp_bcast( eta_stieltjes, ionode_id )

  call mp_bcast( memfac, ionode_id )

  call mp_bcast( kinetic_only, ionode_id )
  call mp_bcast( local_only, ionode_id )
  call mp_bcast( nonlocal_only, ionode_id )
  call mp_bcast( smatrix_only, ionode_id )

  call mp_bcast( eigvec_output, ionode_id )
  ...
  if( readplotspec ) then
    call plotspec_bcast( plotspec, ionode_id )
  endif
  if( readqpts ) then
