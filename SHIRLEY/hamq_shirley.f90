...
! ---------------------------------------------------------------------- 
  ...
  use bspline, only : dbsnak, dbs3in
...
!    haminf_file=trim(prefix)//'.haminf'
  ...
  call mp_bcast( hamloc_file, root )
  call mp_bcast( hamprj_file, root )
...
! ---------------------------------------------------------------------- 
  ...
  use mp, only : mp_scatter_size, mp_scatter
  use mpio, only : mp_file_scatter_write
  use parallel_include
...
! ---------------------------------------------------------------------- 
  ...
  use kinds, only : dp
  use mp_global, only : mpime, root
  use mp, only : mp_bcast, mp_scatter_size, mp_scatter_displ, mp_gather
...
! ---------------------------------------------------------------------- 
  ...
  use mp_global, only : root, nproc
  use mp, only : mp_scatter_size, mp_scatter_displ, mp_scatter, &
                 mp_max, mp_sum, mp_bcast, mp_gather
  use mpio, only : mp_file_scatter_read
  use hamq_pool, only : nproc_per_pool, &
                        cyclic_localindex, &
                        mypool, rootpool, cross_pool_comm, &
...
! ---------------------------------------------------------------------- 
  ...
  ! system details
  call mp_bcast( nelec, root )
  call mp_bcast( alat, root )
  call mp_bcast( omega, root )
  call mp_bcast( at, root )
  call mp_bcast( bg, root )
  call mp_bcast( tpiba, root )
  call mp_bcast( ncpp, root )
  call mp_bcast( nspin, root )
  call mp_bcast( lda_plus_u, root )

  ! transformation matrices
  call mp_bcast( trkin2nlp, root )
  call mp_bcast( trnlp2kin, root )

  write(stdout,*) ' bcast_hamdim: transf matrices'

  ! size of basis
  call mp_bcast( nbasis, root )

  write(stdout,*) ' bcast_hamdim: dims'

  call mp_bcast( nproj, root )
  call mp_bcast( nproj_nl, root )

  if( nproj > 0 ) then

    ! atomic projectors
    call mp_bcast(natom, root )
    call mp_bcast(ntype, root )
    call mp_bcast(nproj_type_max, root )
    ...
    call mp_bcast(type_atom, root )
    call mp_bcast(nproj_type, root )
    call mp_bcast(nproj_type_nl, root )

    call mp_bcast(index_nlproj_type, root )
    call mp_bcast(index_betaq, root )
    call mp_bcast( index_nlproj_betaq, root )

    ! nonlocal spline info
    call mp_bcast( kxord, root )
    call mp_bcast( kyord, root )
    call mp_bcast( kzord, root )
    call mp_bcast( nxcoef, root )
    call mp_bcast( nycoef, root )
    call mp_bcast( nzcoef, root )
    ...
    ! knots
    call mp_bcast( xknot, root )
    call mp_bcast( yknot, root )
    call mp_bcast( zknot, root )

    write(stdout,*) ' bcast_hamdim: knots'

    ! LDA+U
    if( lda_plus_u ) then
      call mp_bcast( natomproj, root )
      call mp_bcast( Hubbard_lmax, root )
      if( mpime/=root ) allocate( Hubbard_l(ntype), Hubbard_U(ntype), Hubbard_alpha(ntype), index_ldaUq(2*Hubbard_lmax+1,natom) )
      call mp_bcast( Hubbard_l, root )
      call mp_bcast( Hubbard_U, root )
      call mp_bcast( Hubbard_alpha, root )
      call mp_bcast( index_ldaUq, root )
...
! ---------------------------------------------------------------------- 
  ...
  do iatom=1,natom
    call mp_bcast( nloper(iatom)%matrix, root )
  enddo
...
! ---------------------------------------------------------------------- 
  ...
  use bspline, only : dbsint, dbs2in, dbs3in
...
! ---------------------------------------------------------------------- 
  ...
  use mp_global, only : mpime
  use bspline, only : dbsval, dbs2vl, dbs3vl
...
! ---------------------------------------------------------------------- 
  ...
  use bspline, only : dbsder, dbs2dr, dbs3dr
