...
! ---------------------------------------------------------------------- 
  ...
  USE uspp, ONLY: nkb, vkb, dvan, okvan, deeq, nhtol, qq
! davegp
  USE us, ONLY: dq, tab
...
! davegp
  ...
  use mp,        only : mp_max, mp_sum, mp_scatter_size, mp_scatter, mp_barrier, mp_bcast, mp_root_sum
  use mp_global, ONLY : mpime, root, nproc, intra_pool_comm
...
!  END IF
  ...
  do ixyz=1,3
    gtmp(1:npw) = g(ixyz,igk(1:npw))*tpiba
  ...
  do ip=1,nproc
    ...
    do ij=1,(nbnd*(nbnd+1))/2
      ...
      if( ijl==1 .or. i==1 ) then
        ...
        psic(nls(igk(1:npw))) = evc(1:npw,ibnd_indx(j))
        ...
        !
        psic(1:nrxxs) = psic(1:nrxxs) * vrs(1:nrxxs,ispin)
        !
        ...
        !
        jtmp(1:npw) = conjg(psic(nls(igk(1:npw))))
        !
    ...
    if( okvan ) then
      do na=1,nat
        ...
        if( upf(nt)%tvanp ) then
          do jh=1,nh(nt)
            do ih=1,nh(nt)
              usq_atom(na)%matrix(ih,jh)=qq(ih,jh,nt)
            enddo
          enddo
        endif
      enddo
...
!    write(stdout,*) '...becp allocated'
    ...
    do ik=1,nkr
      ...
      ! CALL init_us_2( npw, igk, xk_cart(1:3,ik), vkb )
      call init_us_2_shirley( npw, igk, xk_cart(1:3,ik), vkb )
...
!        call calbec( npw, vkb, evc(:,band_subset(1)+ibnd:band_subset(1)+ibnd+nbnd_ip), becp, nbnd_ip )
        ...
        call mp_root_sum( becp%k(:,1:nbnd_ip), becp_l, ip-1 )
...
!    call update_atomic_proj( natomwfc, nat, ntyp, ityp, nhm, nh, nhtol, local_channel )
    ...
    do ik=1,nkr
      ...
      call orthoatwfc_shirley( npw, igk, xk_cart(1:3,ik) )
