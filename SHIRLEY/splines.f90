...
! ----------------------------------------------------------------------
  subroutine bcast_spline_fit( fit, mpime, root )
! ----------------------------------------------------------------------
  ...
  call mp_barrier
  call mp_bcast( fit%xorder, root )
  call mp_bcast( fit%yorder, root )
  call mp_bcast( fit%zorder, root )
  call mp_bcast( fit%nxcoef, root )
  call mp_bcast( fit%nycoef, root )
  call mp_bcast( fit%nzcoef, root )
  ...
  call mp_bcast( fit%xknot, root )
  call mp_bcast( fit%yknot, root )
  call mp_bcast( fit%zknot, root )

  if( mpime==root ) then
    n=size(fit%coefR,1)
    m=size(fit%coefR,2)
  endif
  call mp_bcast( n, root )
  call mp_bcast( m, root )
  ...
  do j=1,m
  do i=1,n
    call mp_bcast( fit%coefR(i,j)%bscoef, root )
    call mp_bcast( fit%coefI(i,j)%bscoef, root )
  enddo
  enddo
