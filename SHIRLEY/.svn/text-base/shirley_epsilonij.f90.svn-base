...
#include "f_defs.h"
  ...
  if( ionode ) then
    ...
    ierr=1
    do while( ierr/=0 )
      if( ntriple==1 ) call errore('shirley_epsilonij','unable to allocate space for triples',1)
  ...
  if( .not. ionode ) then
    allocate( triple_i(neig,neig,ntriple), stat=ierr )
    if( ierr/=0 ) call errore('shirley_epsilonij','unable to allocate space for triples off ionode',1)
  endif
...
! ======================================================================
! Begin calculation of body epsilonij(q->0) 
! ======================================================================
...
! ======================================================================
  ...
  do ik=1,kpt%list%nk
    ...
    if( nocc+npar+nemp /= neig ) call errore('shirley_epsilonij','nocc+npar+nemp /= neig',1)
    ...
    ! accumulate epsij
    write(stdout,*) ' accumulate epsij'
    call ZGEMM( 'C', 'N', neig, neig, nx1(2), fac, xvec2, nx1(2), fxvec2, nx1(2), ONE, epsij, neig )
    write(stdout,*)
...
! ======================================================================
  ...
  ! sum epsij over processors
  write(stdout,*) ' sum over processors'
  call mp_sum( epsij )

  ! multiply by interaction
  call ZGEMM( 'N', 'N', neig, neig, neig, ONE, epsij, neig, vint0, neig, ZERO, ztmp, neig )
  call ZGEMM( 'C', 'N', neig, neig, neig, ONE, vint0, neig, ztmp, neig, ZERO, epsij, neig )
  
  ! dump to file
  if( ionode ) then
    write(stdout,*) 'epsij diagonals '
    do j=1,neig
      write(stdout,'(i,2f)') j, epsij(j,j)
    enddo
    iuneps = freeunit()
    open(iuneps,file=trim(epsijfile),form='unformatted')
    write(iuneps) epsij
    close(iuneps)
  endif

  deallocate( focc )
  deallocate( epsij, ztmp )

  end subroutine body


  end program shirley_epsilonij
