...
! ---------------------------------------------------------------------- 
  ...
  type(spline_dataset_type) :: qmin_dataset
  type(spline_fit_type) :: qmin_fit

  WRITE( stdout, '(/5x,"Calling qinteraction .... ",/)')
  ...
  ! allocate space for spline data 
  call create_spline_dataset( 1, 1, nkgrid, ikgrid, &
                              bg*tpiba, transpose(at)/tpiba, &
                              qmin_dataset )
  ! allocate space for spline fit
  call create_spline_fit( 1, 1, ksplord, qmin_dataset, qmin_fit )
...
!  if( lsda ) current_spin = isk(1)
  ...
  write(stdout,*)
  write(stdout,*) ' construct qinteraction:'
  write(stdout,*)

  ! ======================================================================
  ! qinteraction matrix elements
  ! ======================================================================
  write(stdout,*) ' qinteraction matrix elements'

  ! k-point grid - uniform only
  write(stdout,*) ' generate qinteraction matrices on the following k-point grid:'
  ...
  do ik=1,nkr
    ...
    write(stdout,*)
    write(stdout,*) ' load qinteraction for ik = ', ik, ' of ', nkr
    call flush_unit(stdout)
    ...
    ! no need for rescaling when we multiply 1/qg by q2min anyway
    !qg = qg * (tpiba**2.d0)
    do ig=gstart,npw
      ! including factor of q2min (which could be zero)
      qg(ig) = sqrt( fpi / qg(ig) / omega * q2min )
    enddo
    if( gstart==2 ) then
      qg(1) =  sqrt( fpi / omega )
    endif
    ...
    enddo
    qmin = sqrt(q2min)
    call putelement_spline_dataset( 1, 1, ik, qmin, qmin_dataset )
  ...
  call fit_spline_to_dataset( qmin_dataset, qmin_fit )
  write(stdout,*) ' ...done'

  ! dump to disk
  if( ionode ) then
    iunint = freeunit()
    call seqopn(iunint,'qintrxn','unformatted',exst)
    write(stdout,*)
    write(stdout,*) ' dumping qinteraction to file ', trim(prefix) // '.qintrxn'

    call dump_spline_dataset( qmin_dataset )

    call write_spline_fit( iunint, vint_fit )
    call write_spline_fit( iunint, qmin_fit )
  endif
  ...
  write(stdout,*) ' Done with qinteraction'
  return

  end subroutine qinteraction
! ---------------------------------------------------------------------- 
