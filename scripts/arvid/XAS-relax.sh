...
MyDir=`pwd`
RelaxOut=${MyDir}/relax/${MOLNAME}.relax.out
...
if test ! -d $MyDir/XAS/relax ; then
mkdir $MyDir/XAS/relax
fi
...
if test ! -d $MyDir/XAS/relax/${ATOM_SYMBOL[$i]}${i} ; then
mkdir $MyDir/XAS/relax/${ATOM_SYMBOL[$i]}${i}
fi

#get the last atomic positions of from the relaxation
ATOM_POS=
ATOM_POS=`grep -A $NAT 'ATOMIC_POSITIONS' < $RelaxOut |sed -e '/--/d' | tail -n $((NAT + 1))`
...
SCFOUT=$MyDir/XAS/relax/${ATOM_SYMBOL[$i]}${i}/${MOLNAME}.${ATOM_SYMBOL[$i]}${CATOM}-${CHAPPROX}.scf.out
NSCFOUT=$MyDir/XAS/relax/${ATOM_SYMBOL[$i]}${i}/${MOLNAME}.${ATOM_SYMBOL[$i]}${CATOM}-${CHAPPROX}.nscf.out
BASISOUT=$MyDir/XAS/relax/${ATOM_SYMBOL[$i]}${i}/${MOLNAME}.${ATOM_SYMBOL[$i]}${CATOM}-${CHAPPROX}.basis.out
HAMOUT=$MyDir/XAS/relax/${ATOM_SYMBOL[$i]}${i}/${MOLNAME}.${ATOM_SYMBOL[$i]}${CATOM}-${CHAPPROX}.ham.out
XASOUT=$MyDir/XAS/relax/${ATOM_SYMBOL[$i]}${i}/${MOLNAME}.${ATOM_SYMBOL[$i]}${CATOM}-${CHAPPROX}.xas.out
...
rm -f $MyDir/XAS/relax/${ATOM_SYMBOL[$i]}${i}/Node*
${GeNo}  $MyDir/XAS/relax/${ATOM_SYMBOL[$i]}${i} $NodePool
ls $MyDir/XAS/relax/${ATOM_SYMBOL[$i]}${i}/Node*
${BibDir}/DoXAS.sh $MyDir/XAS/relax/${ATOM_SYMBOL[$i]}${i} $i $XASJOB &
