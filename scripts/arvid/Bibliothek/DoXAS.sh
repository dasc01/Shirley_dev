...
&system
    ...
    ecutwfc =$TMP_ECUT_WFC, ecutrho=$TMP_ECUT_RHO, assume_isolated=$TMP_assume_isolated, input_dft=$TMP_input_dft,
    $TMP_SPIN
    $TMP_TMAG
    $TMP_LDAU
/
...
if [ -z $TMP_DEGAUSS_STEP ]; then
  ...
  then
    ...
    while [[ $count -lt 10 ]]; do
      ...
      if [ $count -gt 5 ] ; then
       echo "Bad convergence: Increasing TMP_ELEC_CONV_THR"
       TMP_ELEC_CONV_THR=1.0D-4
      fi
