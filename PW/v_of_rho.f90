...
SUBROUTINE v_of_rho( rho, rho_core, rhog_core, &
  ...
  USE scf,              ONLY : scf_type, dna, drhoa, vna, dvna  !DAS
  !DASb
  ...
  !
  !DASb
  if(use_psolver .and. scf_psolver) then
     ...
     do is=1,nspin
        ...
        charge=SUM(rho%of_r(:,is))
     enddo
     ehd=ehd*omega/ ( dfftp%nr1*dfftp%nr2*dfftp%nr3 )
     charge=charge*omega/ ( dfftp%nr1*dfftp%nr2*dfftp%nr3 )
     CALL mp_sum(  ehd , intra_pool_comm )
     CALL mp_sum(  charge , intra_pool_comm )
     ehart=ehh+ehd+edd
     do  is=1,nspin
        v%of_r(:,is)=v%of_r(:,is)+vna%of_r(:,is)+dvna%of_r(:,is)
     enddo
     write(6,'(A,5F14.8)') "ehart,ehh,ehd,edd,charge=",ehart,ehh,ehd,edd,charge
  else
  !DASe
