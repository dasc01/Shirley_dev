...
MODULE becmod
  ...
  INTERFACE dump_becp
     !
     MODULE PROCEDURE dump_becp_k, dump_becp_gamma, dump_becp_nc, dump_becp_type
     !
  END INTERFACE
...
!begin DAS mod
!--------------------------------------
  SUBROUTINE DUMP_BECP_TYPE(betapsi,ik,is)
    IMPLICIT NONE
    TYPE (bec_type), INTENT (inout) :: betapsi 
    INTEGER, INTENT (in) :: ik, is
    IF ( gamma_only ) THEN
       CALL dump_becp_gamma (betapsi%r, ik, is)
       !
    ELSEIF ( noncolin) THEN
       !
       CALL dump_becp_nc (betapsi%nc, ik, is)
       !
    ELSE
       !
       CALL dump_becp_k (betapsi%k, ik, is)
       !
    ENDIF
    !
    RETURN
...
!--------------------------------------
  subroutine dump_becp_gamma(betapsi,ik,is)
    ...
    implicit none
    REAL (DP), INTENT (in) :: betapsi(:,:)
    INTEGER::iunit, nkb, nbnd, ikb, ib
    INTEGER, INTENT (in) :: ik, is
    nkb=size(betapsi, 1)
    nbnd=size(betapsi, 2)
    if(mpime==root) then
  ...
  end subroutine dump_becp_gamma
!--------------------------------------
  subroutine dump_becp_nc(betapsi,ik,is)
    USE mp_global, ONLY : intra_pool_comm
    implicit none
    COMPLEX (DP), INTENT (in) :: betapsi(:,:,:)
    INTEGER, INTENT (in) :: ik, is
    write(*,*) betapsi
...
!       open(iunit,file='becp.dmp',form='formatted',status='append')
       write(becunit,'(4I12)') ik, is, nkb, nbnd
       do ib=1, nbnd
          write(becunit,'(2F18.14)') (betapsi(ikb,ib),ikb=1,nkb)
       enddo
!       close(iunit)
    endif
  end subroutine dump_becp_k
!--------------------------------------
!end DAS mod
END MODULE becmod
