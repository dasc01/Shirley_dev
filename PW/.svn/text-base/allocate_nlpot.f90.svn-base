...
subroutine allocate_nlpot
  ...
  !
  ! This change to allocate zero size in the absence of projectors (nkb=0)
  ! is necessary given the way that calbec computes projections and estimates
  ! the number of projections based on the size of the input array vkb,
  ! which is ambiguous if not allocated
  !
  ! davegp
  if (nkb > 0) then
